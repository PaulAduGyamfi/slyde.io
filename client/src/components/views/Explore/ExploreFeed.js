import React from 'react'
import pic from "../viewsStyles/imgs/lbj.jpg";
import {DownOutlined,HeartOutlined,MessageOutlined} from "@ant-design/icons";
import './ExploreFeed.scss'

const ExploreFeed = () => {
    return(
       <div className="explorefeedContainer postComponent">
           <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
            <div className="feedCard">
                <div className="feedCard-Left">
                    <div className="profilePicture" style={{backgroundImage: `url(${pic})`, backgroundPosition: "50% 50%", backgroundSize: "cover"}}></div>
                </div>
                <div className="feedCard-Right">
                    <div className="feedCard-header">
                            <div className="feedCard-info">
                                    <div className="feedCardAuthor Name">Lebron James</div>
                                    <div className="feedCardAuthor Tag">@kingjames</div>
                            </div>
                            <div className="feedCard-arrow">
                                    <DownOutlined />
                            </div>
                    </div>
                    <div className="feedCard-body">
                        We’ve teamed up with @solefed to giveaway a brand new Apple Watch Series 3 (GPS, 42mm) Space Gray Aluminium Case with Black Sport Band to ONE lucky winner. 
                    </div>
                    <div className="feedCard-actions">
                        <HeartOutlined />
                        <MessageOutlined />
                    </div>
                </div>
            </div>
       </div>
    )
}

export default ExploreFeed